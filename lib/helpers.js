...
helpers.getStaticAsset = filename => new Promise(async (res, rej) => {
  const publicDir = path.join(__dirname, '/../public/');
  filename = typeof filename == 'string' && filename.length > 0 ? filename : false;

  if (!filename) return rej('A valid filename was not specified');

  let [err, data] = await helpers.to(readFileAsync(publicDir + filename));
  if (err) return rej('No file could be found');
...
// eliminates need for try/catch or .catch with async/await
